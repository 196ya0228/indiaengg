...
const VideoCMS = () => {
  ...
  const loadVideos = () => {
    ...
    if (savedVideos) {
      ...
      // Initialize with sample videos and save them
      ...
      localStorage.setItem('iew_videos', JSON.stringify(sampleVideos));
    }
  };
  ...
  // Optimized form field updater to prevent re-rendering
  ...
  const generateThumbnail = (): string => {
    if (formData.youtubeUrl) {
      ...
      if (videoId) {
        // Try high resolution first, fallback to medium if not available
  ...
  const autoGenerateThumbnail = useCallback(() => {
    if (formData.youtubeUrl && !formData.thumbnail) {
      const thumbnail = generateThumbnail();
      updateFormField('thumbnail', thumbnail);
      showAlert('Thumbnail automatically generated from YouTube!', 'success');
    }
  }, [formData.youtubeUrl, formData.thumbnail, showAlert]);
  ...
  useEffect(() => {
    if (formData.youtubeUrl && !formData.thumbnail) {
      ...
      }, 1000); // Delay to avoid triggering on every keystroke

      return () => clearTimeout(timer);
    }
  }, [formData.youtubeUrl, formData.thumbnail, autoGenerateThumbnail]);
  ...
  }, [formData, videos, showAlert, resetForm]);
  ...
  }, [formData, editingVideo, videos, showAlert, resetForm]);
  ...
  const openEditDialog = useCallback((video: VideoItem) => {
  ...
  const toggleVideoStatus = useCallback((id: string) => {
  ...
  const duplicateVideo = useCallback((video: VideoItem) => {
    ...
    const updatedVideos = [...videos, duplicatedVideo];
    setVideos(updatedVideos);
        ...
        <div>
          ...
          <div className="flex gap-2">
            <Input
              ...
              onChange={(e) => updateFormField('thumbnail', e.target.value)}
          ...
          {formData.thumbnail && (
            <div className="mt-2">
              <img
                ...
                onError={(e) => {
                  // If maxresdefault fails, try medium resolution
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage product videos - changes reflect immediately on website</p>
        </div>
