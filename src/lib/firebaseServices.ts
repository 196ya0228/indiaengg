...
export const getSocialMediaConfig = async (): Promise<SocialMediaConfig | null> => {
  if (!isFirebaseConfigured()) {
    console.log('Using localStorage for social media config');
  ...
  try {
    ...
    if (docSnap.exists()) {
      return { id: docSnap.id, ...docSnap.data() } as SocialMediaConfig;
    }
    return null;
  } catch (error) {
...
export const updateSocialMediaConfig = async (config: Partial<SocialMediaConfig>): Promise<void> => {
  if (!isFirebaseConfigured()) {
    console.log('Mock mode: Social media config update simulated');
    localStorage.setItem('iew_social_media_config', JSON.stringify({
      ...config,
